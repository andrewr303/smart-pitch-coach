...
const EmptyState: React.FC<EmptyStateProps> = ({ onFileSelect, isProcessing, progress }) => {
  const features = [
    {
      icon: Sparkles,
      ...
      <div className="max-w-2xl w-full">
        ...
        >
          {features.map((feature, index) => (
            <div
              key={feature.title}
              className="p-4 rounded-xl bg-card border border-border hover:border-primary/30 transition-all duration-300 hover:shadow-md"
            >
